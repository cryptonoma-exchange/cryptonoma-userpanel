...
{	
	...
	public function sendBinanceWithdraw($asset, $address, $amount, $addressTag, $network_type)
    {
        $details = array();

        if($network_type != '' && $addressTag != '')
        {
            $details = $this->api->withdraw($asset, $address, $amount, $addressTag, "APIWithdraw", FALSE, $network_type);
        }
        else if($addressTag != '')
        {
            $details = $this->api->withdraw($asset, $address, $amount, $addressTag);
        }
        else if($network_type != '')
        {
            $details = $this->api->withdraw($asset, $address, $amount, NULL, "APIWithdraw", FALSE, $network_type);
        }
        else
        {
           $details = $this->api->withdraw($asset, $address, $amount); 
        }
